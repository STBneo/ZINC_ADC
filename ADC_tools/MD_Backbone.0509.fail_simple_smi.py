...
def Extract_Inner_Scaffold(asmi):
    ...
    for index,row in tmp_df.iterrows():
        if int(row[0]) > 1:
            ...
            tc_smi = Convert_SMILES_to_SIMPLE(c_smi)
            tmp_mol = Chem.MolFromSmiles(tc_smi)
            ...
            for l_idx in range(idx+1,ln_tmp_df):
                ...
                tp_smi = Convert_SMILES_to_SIMPLE(p_smi)
                patt = Chem.MolFromSmiles(tp_smi)
    ...
    tmy_smi = Convert_SMILES_to_SIMPLE(my_smi)
    tm = Chem.MolFromSmiles(tmy_smi)
    print 'Is terminal ring'
    for index,row in tmp_df.iterrows():
        #print 'orig:',row[0],row[1]
        re = Is_terminal_Ring(tm,row[1],dic_atom_neig_idx)
        if re == -1 or re == 0:
            df = df.drop(df[df.smiles==row[1]].index)
    ...
    for index,row in tmp_df.iterrows():
        ...
        match_list = tm.GetSubstructMatches(patt)
        print match_list
...
def Is_terminal_Ring(rdmol,asmi,dic_atom_neig_idx):
    ...
    sma = Chem.MolToSmarts(patt)
    patt = Chem.MolFromSmarts(sma)
    match_list = rdmol.GetSubstructMatches(patt)
...
def Convert_SMILES_to_SIMPLE(asmi):
...
def main():
    ...
    #print Convert_SMILES_to_SIMPLE('O=C1CCNc2ccccc12')
