...
def yklee_work(a_type):
	...
	for afile in if_list:
		...
		if a_type == 0:
			...
			total_df = working_a2(file_name,asmi,Input_CP)
			if total_df.empty:
				bblist = -1
			else:
				bblist = 1
...
def working_a1(asmi,file_name,afile):
	...
	while(Ok_flag!=1):
		...
		print("Loop %d \n Length of Backbone list : %d"%(idx,ltrl))
		if ltrl == 0 and idx == 5:
			return -1
			break

		if ltrl >= 500 or tmp_mw >= np.float64(700.0) or idx == 10:
			Ok_flag = 1
			if ltrl >= 500:
				re_list = re_list|set(random.sample(list(Total_re_list),500))
			else:
				re_list = re_list|Total_re_list
	return list(re_list)
def working_a2(file_name,asmi,Input_CP):
	#######################################
	# Tier 1.6 Match , Backbone Alignment #
	#######################################
	df_list = []
	df_list_t = []
	df_list2 = []
	...
	wsmi_df = pd.DataFrame().from_dict(Input_CP,orient="index").T
	...
	if re_list == -1:
		...
		re_list = set(re_list)
	Input_Num_Ring,up_MW,low_MW = query_parameters(asmi,mw_percent)
	
	div_keys,div_id,BB_dic = Fetch_BB_Rings(Input_Num_Ring,up_MW,low_MW,DB_Path)

	a = 0
	for ids,keys in zip(div_id,div_keys):
		a += 1
		smiles = []
		Align_DF = AlignM3D(file_name,asmi,ids,keys)
		for i in Align_DF["Query"]:
			smiles.append(BB_dic[i])
		Align_DF["SMILES"] = smiles
		Align_DF.drop("Query",axis=1,inplace=True)
		Align_DF.to_csv(out_csv_path + file_name + '.%d'%a + '.csv',index=False)

	for i in glob.glob(out_csv_path + file_name + "*.csv"):
		df_list.append(pd.read_csv(i))
		os.remove(i)

	fin_df = pd.concat(df_list).sort_values(by="PCScore",ascending=False)
	fin_df.to_csv(out_csv_path + file_name + '.total.csv',index=False)
...
def main():
    ...
    #parser.add_argument('-mins',required=True, default=1, help='The minimum number of scaffold.')
    #parser.add_argument('-img',required=True, choices=['y','n'], default='n',  help='Making the png files')
    parser.add_argument("-t",required=True,default=0,help="Working type\n 0 : Tier 1\n 1 : Tier 1.5\n 2 : Tier 1.6\n")
    args=parser.parse_args()
